...
HOTWORD_DIR = (BASE_DIR / 'hotwords')
HOTWORD_DIR.mkdir(parents=True, exist_ok=True)

path_zh = (HOTWORD_DIR / 'hot-zh.txt').resolve()
path_en = (HOTWORD_DIR / 'hot-en.txt').resolve()
path_rule = (HOTWORD_DIR / 'hot-rule.txt').resolve()
path_kwds = (HOTWORD_DIR / 'keywords.txt').resolve()
...
def observe_hot():
    observer = Observer()
    observer.schedule(HotHandler(), str(HOTWORD_DIR), recursive=False)
    observer.start()
    return observer
