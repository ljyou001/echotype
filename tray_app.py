...
class TrayApp:
    def __init__(self) -> None:
        ...
        try:
            ...
            self.logger.debug('Auto-startup status check failed', exc_info=True)
    ...
    def _auto_start(self) -> None:
        # Check if the server needs to be started automatically
        if self.config.get('model_source', 'local') == 'local' and self.config.get('auto_start_server', True):
            if not self._check_server_running():
                self.logger.info('Server not running, attempting to auto-start...')
        ...
        try:
            ...
            self._show_notification(_('Startup failed'), str(exc), force=True)
            self._listening = False
        self._update_toggle_action()
    ...
    def _auto_launch_server(self) -> None:
        ...
        if entry is None:
            self.logger.warning(_('Server startup file not found'))
            return
        ...
        try:
            ...
            self.logger.info(_('Server auto-started'))
            self._show_notification(_('Server Starting'), _('Loading model, please wait...'))
        except Exception as e:
            self.logger.error(_('Failed to auto-start server: {}').format(e))
    ...
    def _do_restart_listening(self) -> None:
        """Performs a delayed restart of the listening service"""
        self.logger.info(_('Preparing to restart listening service...'))
        try:
            self.backend.restart_listening()
            self._listening = True
            self._update_toggle_action()
            self.logger.info(_('Listening service restarted successfully'))
        except Exception as e:
            self.logger.error(_('Failed to restart listening: {}').format(e))
    ...
    def _start_server(self, checked: bool = False, *, from_settings: bool = False) -> bool:
        _ = checked  # QAction triggers pass a boolean
        ...
        if from_settings:
            ...
            if success:
                QMessageBox.information(None, _('Model Startup'), message)
            else:
                QMessageBox.warning(None, _('Startup Failed'), message)
        return success
    ...
    def _launch_server_process(self, return_progress: bool = False) -> Tuple[bool, str, List[Dict[str, str]]]:
        ...
        if entry is None:
            ...
            self._show_notification(_('Model Startup Failed'), message, force=True)
        ...
        ready = False
        try:
            self._show_notification(_('Loading Model'), _('Starting {}...').format(entry.name), force=True)
            if entry.suffix.lower() == '.exe':
            ...
            message = str(exc) or _('Model startup failed')
            self._show_notification(_('Model Startup Failed'), message, force=True)
            return False, message, progress
        else:
            if return_progress and not ready:
                message = _('Model startup timed out. Please check logs or try again later.')
                self._show_notification(_('Model Startup Failed'), message, force=True)
                return False, message, progress
            message = _('Model service has started, please wait for initialization to complete.')
    ...
    def _apply_auto_startup(self, enabled: bool) -> None:
        try:
            ...
            self.logger.warning(_('Failed to configure auto-startup: %s'), exc)
            self._show_notification(_('Auto-startup config failed'), str(exc), force=True)
    ...
    def _list_audio_devices(self) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            self.logger.warning(_('Could not get audio devices: %s'), exc)
        return devices
